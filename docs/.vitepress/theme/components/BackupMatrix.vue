...
const rows = computed<BackupRow[]>(() => props.rows ?? (locale.value === 'en'
  ? [
      { medium: 'One paper copy', protects: 'Affordable and offline', recovery: 'Vulnerable to fire, moisture, and loss', tradeoff: 'Do not use as the only backup' },
      { medium: 'One metal copy', protects: 'More resistant to heat and moisture', recovery: 'Risk of loss, theft, and recording errors', tradeoff: 'A supplementary long-term backup' },
      { medium: 'Paper + metal', protects: 'Distributes different risks', recovery: 'Manage both storage locations', tradeoff: 'A supplementary backup used with the card' },
      { medium: 'Several SeedKeeper cards', protects: 'PIN protection and recovery tests', recovery: 'Check damage, lockout, and compatibility', tradeoff: 'Encrypted copies in separate locations' }
    ]
  : [
...
<template>
  <div class="backup-matrix" role="region" :aria-label="locale === 'en' ? 'Compare backup media and copies' : '백업 매체와 개수 비교'">
    <div class="backup-matrix__scroll"><table><caption>{{ locale === 'en' ? 'Backup media and copy comparison' : '백업 매체·개수 비교' }}</caption><thead><tr><th>{{ locale === 'en' ? 'Setup' : '구성' }}</th><th>{{ locale === 'en' ? 'Benefit' : '장점' }}</th><th>{{ locale === 'en' ? 'Caution' : '주의점' }}</th><th>{{ locale === 'en' ? 'Suggested use' : '권장 용도' }}</th></tr></thead><tbody>
      <tr v-for="row in rows" :key="row.medium"><th>{{ row.medium }}</th><td>{{ row.protects }}</td><td>{{ row.recovery }}</td><td>{{ row.tradeoff }}</td></tr>
    </tbody></table></div>
    <p class="backup-matrix__note">{{ locale === 'en' ? 'No medium is a complete answer by itself. Before using real funds, test recovery with at least two methods.' : '어떤 매체도 단독 정답은 아닙니다. 실제 자금에 쓰기 전에 최소 두 가지 방식으로 복원 가능성을 시험하세요.' }}</p>
  </div>
</template>
<style scoped>
