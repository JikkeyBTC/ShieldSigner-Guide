...
import { getCardArtwork } from '../../../../src/guide/card-artwork'
...
const visibleCards = computed(() => cards.value.map((card) => ({ ...card, artwork: getCardArtwork(card.id) })))
...
<template>
  <aside class="ss-demo-rail" aria-label="Guide visual chapters">
    <a v-for="card in visibleCards" :key="card.id" class="ss-demo-card ss-reveal vp-raw" :data-card-visual="card.visual" :data-card-asset="card.artwork.key" :style="{ '--card-accent': accentFor(card) }" :href="href(card)" :aria-label="card.displayTitle" :aria-current="isCurrent(card) ? 'page' : undefined" @click="runCardAnimation($event, card)">
      <header>
        <span class="ss-scramble-title">{{ card.displayTitle }}</span>
      </header>
