...
const sectionCards: GuideCard[] = sectionLandings.map((landing, index) => ({
  ...landing,
  id: `section-${landing.id}`,
  order: index + 1,
  type: 'category',
  caption: 'SECTION',
  displayTitle: landing.label
}))

const branchLandingCards: GuideCard[] = branchCards.map((landing, index) => ({
  ...landing,
...
const chapterCards: GuideCard[] = chapters.filter((chapter) => chapter.id !== 'overview').map((chapter) => ({
  ...chapter,
...
const cards: GuideCard[] = [...sectionCards, ...branchLandingCards, ...chapterCards]
...
const runCardAnimation = (event: MouseEvent, card: GuideCard) => {
  ...
  if (card.type === 'category') animate(targets('.ss-demo-category-shape'), { translateY: [10, -5, 0], rotate: [-4, 4, 0], opacity: [.45, 1, 1], delay: stagger(90), duration: 520, ease: 'inOutSine' })
