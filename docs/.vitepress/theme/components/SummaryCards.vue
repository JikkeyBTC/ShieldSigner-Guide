...
const cards = computed(() => {
  ...
  if (path.includes('seedkeeper')) return [
    { label: 'READ', text: english ? 'Understand the difference between the card, PIN, and encrypted export.' : '카드·PIN·암호화 내보내기의 차이를 이해하세요.' },
    { label: 'CHECK', text: english ? 'Prepare a recovery test and a second backup card.' : '복원 테스트와 두 번째 백업 카드를 준비하세요.' },
    { label: 'NEXT', text: english ? 'Keep real seeds off the screen.' : '실제 시드는 화면 밖에 기록하지 않습니다.' }
  ]
  if (path.includes('wallet') || path.includes('transactions')) return [
    { label: 'READ', text: english ? 'Connect public information only to watch-only apps.' : '워치온리 앱에는 공개 정보만 연결합니다.' },
    { label: 'CHECK', text: english ? 'Compare the address, amount, and fee on ShieldSigner.' : '주소·금액·수수료를 ShieldSigner에서 대조합니다.' },
    { label: 'NEXT', text: english ? 'Sign and send only after reviewing the PSBT.' : '검토가 끝난 PSBT만 서명하고 전송합니다.' }
  ]
  return [
    { label: 'READ', text: english ? 'Follow the page from the beginning in order.' : '페이지의 순서를 처음부터 따라가세요.' },
    { label: 'CHECK', text: english ? 'Do not skip verification or safety warnings.' : '검증과 안전 경고를 건너뛰지 않습니다.' },
    { label: 'NEXT', text: english ? 'Choose the next step from the left navigation.' : '다음 단계는 왼쪽 목차에서 선택합니다.' }
  ]
})
