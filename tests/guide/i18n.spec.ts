...
describe('guide locale routing', () => {
  ...
  it('temporarily hides only the requested Korean sections', () => {
    ...
    expect(isGuideRouteHidden('/transactions/receive-guide/', 'ko')).toBe(false)
    expect(isGuideRouteHidden('/ko/transactions/send-guide?from=card#review', 'ko')).toBe(false)
    expect(isGuideRouteHidden('/transactions/sign-psbt/', 'ko')).toBe(false)
